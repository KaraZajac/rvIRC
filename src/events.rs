...
fn handle_insert(key: KeyEvent) -> Option<KeyAction> {
    match (key.code, key.modifiers) {
        ...
        (KeyCode::Delete, _) => Some(KeyAction::InputDelete),
        (KeyCode::Up, _) => Some(KeyAction::InputHistoryUp),
        (KeyCode::Down, _) => Some(KeyAction::InputHistoryDown),
        (KeyCode::Left, KeyModifiers::SHIFT) => Some(KeyAction::InputSelectLeft),
        (KeyCode::Right, KeyModifiers::SHIFT) => Some(KeyAction::InputSelectRight),
        (KeyCode::Home, KeyModifiers::SHIFT) => Some(KeyAction::InputSelectHome),
        (KeyCode::End, KeyModifiers::SHIFT) => Some(KeyAction::InputSelectEnd),
        (KeyCode::Left, _) => Some(KeyAction::InputCursorLeft),
        (KeyCode::Right, _) => Some(KeyAction::InputCursorRight),
        (KeyCode::Home, _) => Some(KeyAction::InputCursorHome),
        (KeyCode::End, _) => Some(KeyAction::InputCursorEnd),
